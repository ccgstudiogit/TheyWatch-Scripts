...
public class StalkPlayer : StalkState
{
    ...
    [SerializeField] private int maxStrategicWayPoints = 8;
    ...
    [SerializeField] private float maxStrategicDistance = 30f;
# if UNITY_EDITOR
    ...
    [SerializeField] private int maxGenericWayPoints = 4;
    ...
    [SerializeField] private float stopAtThisDistanceToPlayer = 10f;
#if UNITY_EDITOR
    public override float stopAtPlayerRange => stopAtThisDistanceToPlayer;
#endif
    [Tooltip("A random wait time is selected based on this curve. A random x-value is selected and then the corresponding y-value is " +
        "the value used for the wait time. The wait time is how long this entity stay in one spot whilst close enough to the player")]
    [SerializeField] private AnimationCurve waitTimeNearPlayer;
    [Tooltip("If enabled, instead of using the waitTimeAnimationCurve to randomly select a time to wait if close enough to the player, " +
        "the timeStoppedNearPlayer value will be used instead")]
    [SerializeField] private bool overrideAnimationCurve;
    ...
    [SerializeField] private float timeStoppedNearPlayer = 8f;
    ...
    [SerializeField] private float lookTrackingSpeed = 10f;
    ...
    [SerializeField] private float timeBeforeBecomingAggressive = 22f;
    ...
    public override void FrameUpdate()
    {
        ...
        {
            // This is its own if statement so that the if the above check is true, none of the below if-else statements are checked
            if (!IsEntityMovingToPos())
            {
                ...
                if (elapsedTimeStoppedNearPlayer > timeStoppedNearPlayer)
                {
                    MoveToWayPoint(GetStrategicWayPointAroundPlayer(maxStrategicWayPoints));
                    elapsedTimeStoppedNearPlayer = 0f;

                    // Get a new waitTime for the next time this entity reaches and pauses at a waypoint
                    if (!overrideAnimationCurve)
                    {
                        timeStoppedNearPlayer = HelperMethods.GetRandomValueFromAnimationCurve(waitTimeNearPlayer);
                    }
                }
            }
        }
        // If not within distance to player and finished moving, pause for a bit in between movements
        else if (!IsEntityMovingToPos() && pauseTimeElapsed < pauseTime)
        {
        ...
        else if (!IsEntityMovingToPos() && distanceToPlayer < maxStrategicDistance)
        {
            MoveToWayPoint(GetStrategicWayPointAroundPlayer(maxStrategicWayPoints));
        }
        ...
        {
            MoveToWayPoint(GetSequentialWayPointTowardsTarget(player, maxGenericWayPoints));
        }
